...
identity tool goes through a swappable provider that defaults to sample data and
switches to on-chain reads once an RPC endpoint is set (`source: "mock"` vs `"rpc"`).

<Note>
The identity **provider** defaults to sample data, but the platform's REST API already
reads the **live ERC-8004 + ERC-8183 contracts on Arc testnet** (`GET
/api/arc/contracts`) with no mocks — see [Build on Arc](/chains/arc).
</Note>
