  ...
  return (
    <section className="w-full px-5 py-20 sm:px-8 sm:py-28" style={{ background: '#F4F1FB' }}>
      <div className="mx-auto max-w-[1100px]">
        ...
        <p className="mt-8 text-xs font-semibold tracking-wide text-foreground/45">Ecosystem signals today</p>
        <div className="mt-4 grid grid-cols-2 gap-4 lg:grid-cols-4">
          {SIGNALS.map(({ value, label, note, icon: Icon, fill }) => (
            ...
            >
              ...
              <div className="mt-0.5 text-xs text-foreground/40">{note}</div>
        ...
        <p className="mt-10 text-xs font-semibold tracking-wide text-foreground/45">A-Identity targets</p>
        <ul className="mt-4 flex flex-col gap-3">
          {TARGETS.map((t) => (
            <motion.li
              {...reveal}
              key={t}
              className="flex items-start gap-3 rounded-2xl border border-accent/15 bg-card p-4"
            >
              ...
              <span className="text-sm leading-relaxed text-foreground/70">{t}</span>
            </motion.li>
          ))}
        </ul>
      </div>
