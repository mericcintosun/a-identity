...
export default function Positioning() {
  return (
    <section id="positioning" className="w-full bg-card px-5 py-20 sm:px-8 sm:py-28">
      <div className="mx-auto max-w-[1100px]">
        ...
        <motion.p {...reveal} className="mt-5 max-w-2xl text-lg leading-relaxed text-foreground/65">
          The rails for <span className="font-semibold text-foreground">humans buying through agents</span>{' '}
          already exist. The open gap is{' '}
          <span className="font-semibold text-foreground">agents transacting with each other</span>:
            ...
            >
              <div
                className={`mb-4 grid h-11 w-11 place-items-center rounded-xl ${
                  col.highlight ? 'bg-accent text-white' : 'bg-foreground/5 text-foreground/70'
                }`}
              >
                <col.icon size={20} />
              </div>
              <h3 className="text-lg font-semibold text-foreground">{col.name}</h3>
              <p className="mt-2 text-sm leading-relaxed text-foreground/60">{col.blurb}</p>
                  ...
                  <li key={t.label} className="flex items-center gap-2.5 text-sm">
                    <span
                      className={`grid h-5 w-5 shrink-0 place-items-center rounded-full ${
                        t.ok ? 'bg-accent/15 text-accent' : 'bg-foreground/8 text-foreground/35'
                      }`}
                    >
                      {t.ok ? <Check size={12} /> : <Minus size={12} />}
                    </span>
                    <span className={t.ok ? 'text-foreground/80' : 'text-foreground/45'}>{t.label}</span>
                  </li>
                ))}
              </ul>
            </motion.div>
        ...
        <motion.p {...reveal} className="mt-8 text-sm text-foreground/50">
